...
class Requests{
  ...
  async execute(Config, Params){
    ...
    for (let ReqIndex = 0; ReqIndex < Object.keys(Config.requests).length; ReqIndex++) {
      ...
      if(CurrentRequest.method === "POST"){
        ...
        if (TokenMatch) {
          let token = Requests.tokens[TokenMatch[1]] ? Requests.tokens[TokenMatch[1]].token : Requests.tokens[0].token 
      ...
      if(CurrentRequest.tokens){
        for (let TokenIndex = 0; TokenIndex < CurrentRequest.tokens.length; TokenIndex++) {
          ...
          let TokenRegex = String(CurrentRequest.tokens[TokenIndex].token).replace('<TOKEN>', '(.+)')
